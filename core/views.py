        ...
        if form.is_valid():
            ...
            try:
                status = apply_new_listing_submission(
                    obj, profile, category, request.user, save_mode=save_mode, ip_address=_client_ip(request),
                )
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            status = apply_listing_edit_state(
                obj, profile, model_key, save_mode=save_mode, ip_address=_client_ip(request),
            )
